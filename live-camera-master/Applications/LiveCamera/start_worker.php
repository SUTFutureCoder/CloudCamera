...
$recv_worker->onMessage = function($connection, $data)use($recv_worker, $objRedis)
{
    if (!is_object($data)){
        if (is_string($data)){
            //字符串
            $strDataRec = json_decode($data, true);
            if ('camera_init' == $strDataRec['type']){
                //进行验证,返回passport
                $connection->send(json_encode(array('valid' => 1)));
                if (empty($strDataRec)){
                    $connection->send(json_encode(array('valid' => 0)));
                }
            }
        } else {
            //二进制
